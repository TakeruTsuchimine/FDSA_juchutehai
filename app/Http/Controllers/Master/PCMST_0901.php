...
class PCMST_0901 extends Controller
{
    public function index(Request $request)
    {
        ...
        try {
            ...
            if(is_null($waritsukeKouhoCd) || $waritsukeKouhoCd === ''){
                $resultMsg .= '「'.__('waritsuke_kouho_cd_kikai').'」'.__('が正常に送信されていません。').'\n';
                $resultFlg = false;
            }
            ...
            switch ($SQLType) {
                ...
                default:
                    ...
                    {
                        $resultMsg .= '「'.__('kikai_cd').'」'.__('が正常に送信されていません。').'<br>';
                        $resultFlg = false;
                    }
                    // コードが存在しない場合はエラー
                    $result = $common -> GetCdCount('kikai_master', 'kikai_cd', $kikaiCd);
                    if($result < 1)
                    {
                        $resultMsg .= __('登録されていない').'「'.__('kikai_cd').'」'.__('です。').'<br>';
                    ...
                    $SQLBind[] = array('kakou_nouryoku',     $kakouSkill,     TYPE_STR);
