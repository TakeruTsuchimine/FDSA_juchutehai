...
<div class="flex-box flex-column" style="padding:5px 10px;">
    ...
    <div class="form-column">
        <!-- 「メニュータブ名（業務名）」 -->
        <label>
            <span style="width:10.4em;">{{__('menu_group_name')}}</span>
            <input name="dataMenuGroupName" class="form-control code-check" type="text" maxlength="16" autocomplete="off"
                style="width:17.5em;" required>
        </label>
    </div>
    <div class="form-column">
        <!-- 「有効期間（自）」 -->
...
<script>
    ...
    var fncExportExcel = function()
    {
        ...
        let columns = [{ binding: 'dataMenuGroupCd', header: "{{ __('menu_group_cd') }}" },
                       ...
                       { binding: 'dataMenuSeqno', header: "{{ __('menu_group_seqno') }}" },
    ...
    var fncNyuryokuData = function(mode, copy)
    {
        ...
        nyuryokuData['dataMenuGroupSeqno'].disabled = deleteFlg;  /* 「メニューGRSEQNO」 */
    ...
    $('#frmNyuryoku').submit(function(event)
    {
        ...
        function IsChangeNyuryokuData(nyuryokuData)
        {
            ...
            /* 「メニューGRCD」 */
            if(nyuryokuData['dataMenuGroupCd'].value != data['dataMenuGroupCd']) return true;
            /* 「有効期間（自）」 */
