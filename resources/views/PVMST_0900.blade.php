...
<script>
    ...
    function InitGrid()
    {
        ...
        let columns = [{ cells: [{ binding: 'dataWaritsukekouhoCd',   header: "{{__('waritsukekouho_cd')}}", width: '2*' }]},
                       ...
                       { cells: [{ binding: 'dataKikaiCd',        header: "{{__('sentoKikai_cd')}}", width: '2*' },]},
                       { cells: [{ binding: 'dataKikaiName',      header: "{{__('sentoKikai_name')}}", width: '2*' },]},
    ...
    var fncShowDataGrid = function()
    {
        ...
        {{-- 検索件数のデータ受信 --}}
        AjaxData("{{ url('/master/0900') }}", soushinData, fncJushinDataCnt);
    }
    {{-- 「CSV出力」ボタンイベント --}}
    var fncExportCSV = function()
    {
        ...
        let columns = [{ binding: 'dataId',   header: "{{__('id') }}" },
               ...
               { binding: 'dataWaritsukekouhoCd',   header: "{{__('waritsukekouho_cd')}}" }, 
               { binding: 'dataSubSeq',   header: "{{__('sub_seq')}}" }, 
               { binding: 'dataWaritsukekouhoName',   header: "{{__('waritsukekouho_name')}}" }, 
               { binding: 'dataSetsumeibun',   header: "{{__('setsumeibun')}}" }, 
               { binding: 'dataKikaiCd',   header: "{{__('kikai_cd')}}" }, 
               { binding: 'dataKakouNouryoku',   header: "{{__('kakou_nouryoku')}}" }, 
    ...
    function sinkiSend(){
        ...
        let soushinDataLeft = {}
        // 非同期データ更新開始
        AjaxData("{{ url('/inquiry/GridLeftKikai') }}", soushinDataLeft,WaritsukeGridLeftSend); // グリッドデータ
    }
    ...
    var WaritsukeGridRightSend = function(data, errorFlg)
    {
        ...
        gridMasterRight = new wijmo.grid.FlexGrid('#gridMasterRight',{itemsSource: data[1],
            // レイアウト設定
            columns: [
                { binding: 'dataKikaiCd', header: "{{__('kikai_cd')}}", width: 100},
                { binding: 'dataKikaiName',  header: "{{__('kikai_name')}}", width:100 },
