    ...
    <div class="form-column">
        ...
        <label>
            ...
            <span class="icon-field"> <!-- アイコンボタンを設置する入力項目は、スタイルクラス「icon-field」を宣言 -->
                <input name="dataTokuisakiCd" class="form-control" type="text" maxlength="10" autocomplete="off"
                    style="width:8em;">
                <!-- 検索アイコンは、スタイルクラス「search-btn」を宣言 -->
                <i class="fas fa-search search-btn"></i>
            </span>
            <!-- フォーカス時のイベントを「onfocus="blur();"」に設定してフォーカス対象を無効にする -->
            <input name="dataTokuisakiName1" class="form-control" type="text" style="width:22em;" onfocus="blur();" readonly>
        </label>
    </div>
    <!-- 表示件数 -->
...
<div class="flex-box flex-column" style="padding:5px 10px;">
    <div class="form-column">
        <!-- 「品目CD」 -->
        <label>
            <span style="width:4.7em;">{{__('hinmoku_cd')}}</span>
    ...
    <div class="form-column">
        <!-- 「得意先CD」 -->
        <label>
            <span style="width:4.7em;">{{__('tokuisaki_cd')}}</span>
            <span class="icon-field"> <!-- アイコンボタンを設置する入力項目は、スタイルクラス「icon-field」を宣言 -->
                <!-- コード検査を行う項目は、スタイルクラス「code-check」を宣言 -->
            ...
            <!-- フォーカス時のイベントを「onfocus="blur();"」に設定してフォーカス対象を無効にする -->
            <input name="dataTokuisakiName1" class="form-control" type="text" style="width:12em;" onfocus="blur();" readonly>
        </label>
    </div>
    <div class="form-column">
        <!-- 「適用日付」 -->
        <label>
            <span style="width:5.4em;">{{__('tekiyou_date')}}</span>
    ...
    <div class="form-column">
        <!-- 「売上単価」 -->
        <label>
            <span style="width:5em;">{{__('uri_tanka')}}</span>
            <input name="dataUriTanka" class="form-control" type="text" maxlength="17" autocomplete="off"
                style="width:8em;">
        </label>
    </div>
    <div class="form-column">
...
<script>
    ...
    var fncShowDataGrid = function()
    {
        ...
        /* グリッドのデータ受信 */
        AjaxData("{{ url('/master/2800') }}", soushinData, fncJushinGridData);
    }
    /* 「CSV出力」ボタンイベント */
    var fncExportCSV = function()
    {
        /* CSV出力用グリッドのレイアウト設定 */
        let columns = [{ binding: 'dataTokuisakiCd', header: "{{ __('tokuisaki_cd') }}" },
                       { binding: 'dataTokuisakiName1', header: "{{ __('tokuisaki_name') }}" },
                       ...
                       { binding: 'dataUriTanka', header: "{{ __('uri_tanka') }}" },
                       ...
                       { binding: 'dataKoushinshashaId', header: "{{ __('koushinsha_id') }}" }];
        /* 現在のグリッドの並び替え条件取得 */
    ...
    var fncNyuryokuData = function(mode, copy)
    {
        ...
        /* 「売上単価」 */
        nyuryokuData['dataUriTanka'].value = copy ? data['dataUriTanka'] : '';
        /* 「旧単価」 */
        ...
        nyuryokuData['dataUriTanka'].disabled = deleteFlg; /* 「売上単価」 */
    ...
    $('#frmNyuryoku').submit(function(event)
    {
        ...
        function IsChangeNyuryokuData(nyuryokuData)
        {
            ...
            /* 「売上単価」 */
            if((nyuryokuData['dataUriTanka'].value != data['dataUriTanka']) &&
              !(nyuryokuData['dataUriTanka'].value == '' && data['dataUriTanka'] == null)) return true;
            /* 「旧単価」 */
        ...
        if(nyuryokuData['dataSQLType'].value == MODE_DELETE)
        {
            ...
            function()
            {
                ...
                /* 非同期データ更新開始 */
                AjaxData("{{ url('/master/2801') }}", soushinData, fncUpdateData);
            }, null);
        }
        else
        {
