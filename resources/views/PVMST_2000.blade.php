...
<form id="frmKensaku" name="frmKensaku" class="flex-box" style="height:{{ $kensakuHight }};">
    ...
    <div class="form-column">
        ...
        <label>
            ...
            <input name="dataMakerCd" class="form-control code-check" type="text" 
        ...
        <label>
            ...
            <input name="dataMakerName" class="form-control code-check" type="text" 
...
<div class="flex-box flex-column" style="padding:5px 10px;">
    <div class="form-column">
        {{-- 「メーカーCD」 --}}
        <label>
            <span style="width:6.7em;">{{__('maker_cd')}}</span>
            <input name="dataMakerCd" class="form-control code-check" type="text" 
                maxlength="10" autocomplete="off" style="width:10em;" 
                    pattern="^([a-zA-Z0-9]{0,10})$" title="{{ ('半角英数字10文字以内で入力してください')}}"   required>
        </label>
    ...
    <div class="form-column">
        {{-- 「略称」 --}}
        <label>
            <span style="width:7.1em;">{{__('maker_ryaku')}}</span>
            <input name="dataMakerRyaku" class="form-control code-check" type="text" 
                maxlength="10" autocomplete="off" style="width:10em;" >
        </label>
    </div>
    <div class="form-column">
        {{-- 「メーカー名」 --}}
        <label>
            <span style="width:6.7em;">{{__('maker_name')}}</span>
            <input name="dataMakerName" class="form-control code-check" type="text" 
                maxlength="10" autocomplete="off" style="width:10em;" required>
        </label>
    </div>
...
<script>
    ...
    {
        {{-- MultiRowのレイアウト設定 --}}
        let columns =   [{ cells: [{ binding: 'dataMakerCd',   header: "{{__('maker_cd')}}", width: '1*'  },]}, 
               { cells: [{ binding: 'dataMakerRyaku',   header: "{{__('maker_ryaku')}}", width: '1*'},]}, 
               { cells: [{ binding: 'dataMakerName',   header: "{{__('maker_name')}}", width: '1*' },]}, 
               { cells: [{ binding: 'dataStartDate',          header: "{{__('yukoukikan_start_date')}}", width: '1*'}] },
               { cells: [{ binding: 'dataEndDate',            header: "{{__('yukoukikan_end_date')}}", width:'1*'}] }]
    ...
    var fncExportCSV = function()
    {
        ...
        let columns = [{ binding: 'dataId',   header: "{{__('id') }}" },
                    ...
                    { binding: 'dataMakerRyaku',   header: "{{__('maker_ryaku')}}" }, 
    ...
    $('#frmNyuryoku').submit(function(event)
    {
        ...
        function IsChangeNyuryokuData(nyuryokuData)
        {
            ...
            {{-- 「略称」 --}}
            if(nyuryokuData['dataMakerRyaku'].value != data['dataMakerRyaku']) return true;
            {{-- 「メーカー名」 --}}
