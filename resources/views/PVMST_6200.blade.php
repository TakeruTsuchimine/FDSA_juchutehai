...
<form id="frmKensaku" name="frmKensaku" class="flex-box" style="height:{{ $kensakuHight }};">
    ...
    <div class="form-column">
        ...
        {{-- 「メニューSEQNO」 --}}
        <label>
            <span style="width:7.5em;">{{__('menu_seq')}}</span>
...
<div class="flex-box flex-column" style="padding:5px 10px;">
        ...
        <label>
            ...
            <span class="icon-field"> {{-- アイコンボタンを設置する入力項目は、スタイルクラス「icon-field」を宣言 --}}
                ...
                <input name="dataJigyoubuCd" class="form-control code-check" type="text"
                        ...
                        pattern="^([a-zA-Z0-9]{0,6})$" title="{{__('半角英数字6文字以内で入力してください')}}" required>
                {{-- 検索アイコンは、スタイルクラス「search-btn」を宣言 --}}
                <i class="fas fa-search search-btn"></i>
            </span>
            {{-- フォーカス時のイベントを「onfocus="blur();"」に設定してフォーカス対象を無効にする --}}
    ...
    <div class="form-column">
        {{-- 「メニューSEQNO」 --}}
        <label>
            <span style="width:7.9em;">{{__('menu_seq')}}</span>
            <input name="dataMenuSeq" class="form-control" type="text" maxlength="3" autocomplete="off"
                style="width:8em;">
        </label>
    </div>
    <div class="form-column">
        {{-- 「メニュータイトル」 --}}
        <label>
            <span style="width:7.9em;">{{__('menu_title')}}</span>
            <input name="dataMenuTitle" class="form-control" type="text" maxlength="16" autocomplete="off"
                style="width:22em;">
        </label>
    </div>
    <div class="form-column">
        {{-- 「メニュー機能URL」 --}}
        <label>
            <span style="width:7.9em;">{{__('menu_title_url')}}</span>
            <input name="dataMenuTitleUrl" class="form-control" type="text" maxlength="512" autocomplete="off"
                style="width:44em; margin-left:5px;">
        </label>
    </div>
    <div class="form-column">
        {{-- 「有効期間（自）」 --}}
...
<script>
    ...
    var fncShowDataGrid = function()
    {
        ...
        {{-- 検索件数のデータ受信 --}}
        AjaxData("{{ url('/master/6200') }}", soushinData, fncJushinDataCnt);
    }
    {{-- 「CSV出力」ボタンイベント --}}
    var fncExportCSV = function()
    {
        ...
        let columns = [{ binding: 'dataMenuGroupCd', header: "{{ __('menu_group_cd') }}" },
                       ...
                       { binding: 'dataMenuSeq', header: "{{ __('menu_seq') }}" },
    ...
    var fncNyuryokuData = function(mode, copy)
    {
        ...
        {{-- 「メニューSEQNO」 --}}
        nyuryokuData['dataMenuSeq'].value = copy ? data['dataMenuSeq'] : '';
        {{-- 「事業部CD」 --}}
        ...
        nyuryokuData['dataMenuGroupSeq'].disabled = deleteFlg; {{-- 「メニューグループSEQNO」 --}}
        nyuryokuData['dataMenuSeq'].disabled = deleteFlg;  {{-- 「メニューSEQNO」 --}}
    ...
    $('#frmNyuryoku').submit(function(event)
    {
        ...
        function IsChangeNyuryokuData(nyuryokuData)
        {
            ...
            {{-- 「メニューSEQNO」 --}}
            if(nyuryokuData['dataMenuSeq'].value != data['dataMenuSeq']) return true;
            {{-- 「メニュータイトル」 --}}
